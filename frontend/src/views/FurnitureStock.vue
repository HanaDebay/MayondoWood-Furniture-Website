<template>
  <div class="container-fluid p-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
      <h2>Furniture Stock</h2>
      <button class="btn btn-primary" @click="showAddModal = true">Add Furniture</button>
    </div>
    ...
    <table class="table table-striped">
      ...
      <tbody>
        <tr v-for="stock in furnitureStocks" :key="stock._id">
          <td>{{ stock.productName }}</td>
          <td>{{ stock.productType }}</td>
          <td>{{ stock.furnitureType }}</td>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">{{ editingStock ? 'Edit' : 'Add' }} Furniture</h5>
          ...
          <form @submit.prevent="saveStock" novalidate>
            <div class="modal-body">
              <div class="mb-3">
                ...
                <input v-model="form.productName" type="text" class="form-control" @input="() => { touchField('productName'); validateField('productName') }" @blur="touchField('productName')" />
                <div v-if="formErrors.productName" class="form-text text-danger">{{ formErrors.productName }}</div>
              </div>
              <div class="mb-3">
                <label class="form-label">Product Type</label>
                <select v-model="form.productType" class="form-select" @change="() => { form.furnitureType = ''; touchField('productType'); validateField('productType') }">
                  <option value="" disabled>Select type</option>
                  <option value="furniture-home">Home Furniture</option>
                  <option value="furniture-office">Office Furniture</option>
                </select>
                <div v-if="formErrors.productType" class="form-text text-danger">{{ formErrors.productType }}</div>
              </div>
              <div v-if="furnitureTypeOptions.length" class="mb-3">
                <label class="form-label">Furniture Type</label>
                <select v-model="form.furnitureType" class="form-select" @change="() => { touchField('furnitureType'); validateField('furnitureType') }">
                  <option value="" disabled>Select furniture type</option>
                  <option v-for="option in furnitureTypeOptions" :key="option.value" :value="option.value">
                    {{ option.text }}
                  </option>
                </select>
                <div v-if="formErrors.furnitureType" class="form-text text-danger">{{ formErrors.furnitureType }}</div>
              </div>
              ...
              <div class="mb-3 mt-3">
                ...
                <input v-model="form.color" type="text" class="form-control" placeholder="e.g., Walnut / Black" />
              </div>

              <div class="mb-3">
                <label class="form-label">Measurements <span class="text-muted">(optional)</span></label>
                <input v-model="form.measurements" type="text" class="form-control" placeholder="e.g., 180x60x75 cm" />
              </div>
            </div>
    ...
    <div v-if="showDeleteModal" class="modal d-block" tabindex="-1" role="dialog" aria-modal="true" style="background: rgba(0,0,0,0.5)">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">Delete Furniture Stock</h5>
...
const validateField = (field) => {
  ...
  switch (field) {
    case 'productName':
      formErrors.value.productName = value && value.toString().trim() ? '' : 'Product name is required.'
      break
    case 'productType':
      formErrors.value.productType = value ? '' : 'Product type is required.'
      break
    case 'furnitureType':
      formErrors.value.furnitureType = value ? '' : 'Furniture type is required.'
      break
    case 'costPrice':
      if (value === '' || value === null) {
    ...
    case 'date':
      if (!value) {
        ...
        formErrors.value.date = isNaN(selectedDate.getTime()) ? 'Select a valid date.' : ''
      }
      break
    case 'quality':
...
const furnitureTypeOptions = computed(() => {
  if (form.value.productType === 'furniture-home') return homeFurniture
  if (form.value.productType === 'furniture-office') return officeFurniture
  return []
})
...
const loadFurnitureStocks = async () => {
  try {
    const { data } = await axios.get('/api/furniture-stock')
    furnitureStocks.value = data.furnitureStocks || data
  } catch (e) {
    error.value = 'Failed to load furniture stocks'
  }
}
...
const saveStock = async () => {
  ...
  try {
    if (editingStock.value) {
      await axios.put(`/api/furniture-stock/${editingStock.value._id}`, form.value)
    } else {
      await axios.post('/api/furniture-stock', form.value)
    }
    closeModal()
    loadFurnitureStocks()
  } catch (e) {
...
const confirmDeleteStock = async () => {
  ...
  deleting.value = true
  try {
    await axios.delete(`/api/furniture-stock/${stockToDelete.value._id}`)
    closeDeleteModal()
    loadFurnitureStocks()
  } catch (e) {
